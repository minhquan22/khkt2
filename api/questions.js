...
export default async function handler(request) {
  ...
  try {
    ...
    if (request.method === 'POST') {
      ...
      // 1. TẠO HASH cho câu hỏi mới
      const newQuestionHash = await generateQuestionHash(body.question, body.options);
      
      // 2. KIỂM TRA DUPLICATE trong Blob Storage
      const duplicateResult = await findDuplicateQuestion(newQuestionHash);
      
      if (duplicateResult.found) {
        // 3. NẾU TÌM THẤY TRÙNG => TRẢ VỀ CÂU HỎI CŨ
        return new Response(
          JSON.stringify({
            success: true,
            duplicate: true,
            message: '⚠️ Câu hỏi tương tự đã tồn tại',
            existingQuestion: duplicateResult.question,
            newQuestionHash: newQuestionHash,
      ...
      // 4. NẾU KHÔNG TRÙNG => TẠO CÂU HỎI MỚI
      ...
      const questionData = {
        ...
        hash: newQuestionHash, // THÊM HASH vào dữ liệu để kiểm tra sau này
      ...
      const blob = await put(
        `questions/${id}.json`,
        JSON.stringify(questionData, null, 2), // Định dạng JSON đẹp
        {
          access: 'public',
          token: process.env.BLOB_READ_WRITE_TOKEN,
